...
%         groupaveragesA(g,:) = mean(SubjAvg.avgRT(SubjAvg.group==g));
      groupaveragesA(g,:) = [unique(SubjAvg.group(SubjAvg.group==g)) mean(SubjAvg.avgRT(SubjAvg.group==g))];        
    end
    
GroupAvg = array2table(groupaveragesA); % turn this info into a table
...
%% Competition type by player 
% playerNumbers = p-1;
s=1;

for s=1:length(SubjID)
    for p=1:length(unique(DataRegretEnvy.players));
        SubjAvg.(['RT' (num2str(p))])(s,:) = mean(DataRegretEnvy.choiceduration(DataRegretEnvy.id==s & DataRegretEnvy.players==p));
        
%         SubjAvg.RT1 = mean(DataRegretEnvy.choiceduration(DataRegretEnvy.id==s & DataRegretEnvy.players==1));
     
        
%         playerNumbers = p-1;
% subjectPlayTypeAverages(p,:) = [unique(DataRegretEnvy.id(DataRegretEnvy.id==s)) ...
%     DataRegretEnvy.id(s)(DataRegretEnvy.players)==playerNumbers unique(DataRegretEnvy.age(DataRegretEnvy.id==s)) ...
%     unique(DataRegretEnvy.group(DataRegretEnvy.id==s)) unique(DataRegretEnvy.sex(DataRegretEnvy.id==s)) ... 
%     unique(DataRegretEnvy.education(DataRegretEnvy.id==s)) mean(DataRegretEnvy.choiceduration(DataRegretEnvy.id==SubjID(s)))];
    end
end
